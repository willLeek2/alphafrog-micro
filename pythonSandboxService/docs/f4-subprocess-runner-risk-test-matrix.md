...
| R6 | **pool 满** 时行为不明（排队 vs 失败 vs 静默变慢） | P1 | 可配置策略 + `pool_acquire_ms` / `pool_rejected` 日志；T6 |
...
| T6 | **pool 耗尽** | 并发 > `max_pool_size` | 行为符合配置（queue/reject）；日志含 **pool_acquire_ms** 或 reject 原因 | 无无限 hang |
...
- `task_id`, `container_id`, `pool_acquire_ms`, `workspace_prepare_ms`, `copy_ms`, `subprocess_run_ms`, `cleanup_ms`, `total_ms`
