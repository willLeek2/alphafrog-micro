...
public class AdminFetchJobExpansionService {
    ...
    private List<LeafTask> expandSingleTaskSet(Map<String, Object> rawTask, int index) {
        ...
        switch (expandStrategy) {
            ...
            case "fund_batches" -> {
                // fund_batches: 按本地场外基金分批展开，使用 fund_offset/fund_batch_limit 区分于 TuShare 分页 offset/limit。
                int baseOffset = getIntValue(baseParams.get("fund_offset"), 0);
                int batchSize = readFundBatchSize(baseParams);
                int fundCountLimit = getIntValue(baseParams.get("fund_count_limit"), batchSize);
                if (fundCountLimit <= 0) fundCountLimit = batchSize;
                int batchCount = (fundCountLimit + batchSize - 1) / batchSize;
                for (int b = 0; b < batchCount; b++) {
                    Map<String, Object> p = new LinkedHashMap<>(baseParams);
                    p.put("fund_offset", baseOffset + b * batchSize);
                    p.put("fund_batch_limit", batchSize);
                    expandedParamsList.add(p);
                }
            }
            case "date_range_with_fund_batches" -> {
                // date_range_with_fund_batches: 固定日期范围 x 本地场外基金批次笛卡尔积展开
                @SuppressWarnings("unchecked")
                Map<String, Object> dateRange = (Map<String, Object>) rawTask.get("date_range");
                if (dateRange == null) {
                    throw new IllegalArgumentException("task_sets[" + index + "] date_range_with_fund_batches 模式需要 date_range 配置");
                }
                Object startDateRaw = dateRange.get("start_date");
                Object endDateRaw = dateRange.get("end_date");
                if (startDateRaw == null || endDateRaw == null) {
                    throw new IllegalArgumentException("task_sets[" + index + "] date_range_with_fund_batches 模式需要 start_date 和 end_date");
                }
                int baseOffset = getIntValue(baseParams.get("fund_offset"), 0);
                int batchSize = readFundBatchSize(baseParams);
                int fundCountLimit = getIntValue(baseParams.get("fund_count_limit"), batchSize);
    ...
    /**
     * task_sets 中与「本地场外基金每批条数」对应的参数：{@code fund_batch_limit}；
     * 不兼容 {@code limit}，因为 {@code limit} 在 fund_portfolio 中表示每个 TuShare 请求的返回上限。
     */
    private int readFundBatchSize(Map<String, Object> baseParams) {
        int batchSize = getIntValue(baseParams.get("fund_batch_limit"),
