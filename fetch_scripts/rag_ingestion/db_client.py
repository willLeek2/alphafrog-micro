...
class DbClient:
    ...
    def _ensure_login(self) -> None:
        """惰性登录：首次调用 records 端点前获取管理员用户 JWT。"""
        if self._jwt:
            return
        self._cfg.login_username = self._login_username
        self._cfg.login_password = self._login_password
        self._jwt = get_admin_jwt(self._cfg, self.session)
