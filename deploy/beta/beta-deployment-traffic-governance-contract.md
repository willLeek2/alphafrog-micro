...
候选已经被健康检查明确判为失败、提供者注册在期限内始终不可见或就绪超时时，控制器停止并清理候选容器；提供者候选进程退出时由 Dubbo 自行注销，旧活动实例继续服务。Nacos 查询失败时保留已经记录的候选并进入 `FAILED`；容器身份冲突时停止自动操作并记录失败，不承诺一定存在可保留的候选。这两类失败都要等待人工核对后显式重试。控制器崩溃后依据操作阶段和容器事实继续，不重新创建已经记录的候选。
...
一次部署只允许一个未完成操作。控制器串行执行操作，并在外部步骤后的状态写入前重新核对 `operationId` 和操作阶段；容器步骤按相应阶段核对持久化容器身份。候选就绪阶段还只读确认目标地址和端口可见。发现容器身份冲突或无法查询候选可见性时停止，不猜测成功。
...
控制器在启动容器前用 `docker compose config` 读取有效配置，并在有效配置与计划不一致时拒绝启动。

## 9. 重启恢复

控制器重启后先读取并校验部署单、全局状态和容器现场：

- 候选计划存在但容器不存在：按确定名称继续创建。
- 候选容器存在且身份一致：继续等待容器健康；提供者服务还要等待自注册端点可见，然后提升候选。
- 服务处于切换检查点：按持久状态提升候选并把旧实例移入排空记录，不创建第二个候选。
- 服务处于排空阶段：旧容器存在则按原截止时间继续停止，不存在则清理记录。
- 候选可见性查询失败或容器身份冲突：进入 `FAILED` 并等待显式重试。
...
- 控制器重启时能继续候选健康确认、角色提升和排空；停止截止时间不因重启或重试延长；不创建重复候选，不认领身份不一致对象。
