...
状态：观测字段与隐私规则合同（执行方 cursor-bob-mbp）。基线提交 `5dccbef7`（分支 `refactor/agent-codebase-refine`）。
消费方：compose 单写者（五字段与部署预检）、日志 JSON 化、采集器透传、单服务试点验收（五字段与分部署查询）、部署控制器（beta 侧生成与字符校验）、后续观测线端到端验收与只读观测工具。
变更纪律：下面「五字段清单」是定义、生成、验收三处共用的同一份表。改字段名、改取值规则或改验收方法，必须同步改这三处，以及所有消费方。字段名是对外契约。

依据：开工计划一中的观测字段与隐私规则条目；落地版上篇 §1.2 / §1.3 / §1.4 / §1.6 / §1.7；中篇 §3(b) 第 1 步与两机连通清单；开工计划二 §一 第 1、2、6 条。隐私口径由 frog 确认后生效（计划二 §一 第 6 条）。
...
### 2.1 稳定实例：compose 单写者要写进 compose 的字符串
...
### 2.3 部署预检（生产与 beta；compose 单写者实现，部署控制器在 beta 创建路径复用）
...
把左列填进 `image.digest` 会让单服务试点和泳道端到端验收的「与容器实际 Image ID 精确相等」整批失败，也会让 11 个服务看起来像用了同一份镜像。compose 单写者和部署控制器生成资源属性时只读右列。
...
- 禁止：逗号 `,`、等号 `=`、换行 `\n`、回车 `\r`。
- 禁止空值。
- `deployment.id` 另外遵守 Beta 部署与流量治理合同规定的 deployment-id 字符集与长度；本清单要求它必须同时满足上一条。该字符集若尚未合入，生成端先按「小写字母、数字、连字符，长度 1–63，不能以连字符开头或结尾」执行，并与 Beta 部署合同对齐一次。保留字 `stable` 仅稳定实例使用。
...
日志 JSON 里的键名是 `deployment`，不是 `deployment.id`。这是落地版 §1.3 的 logback `customFields` 写法，日志 JSON 化按它抄。
...
| Agent 退役凭证 | `AF_DEPLOYMENT_RETIREMENT_TOKEN` 及 `RetireDeploymentGeneration` 请求中的 `retirement_token`；不得写日志、trace、状态接口、部署单、全局状态或 RPC 响应 |
...
javaagent（compose 挂上时保持这些默认，不要为排查打开）：
...
用户问题、SQL 结构（已脱敏的 `db.statement`）、内部 `userId`、Run / trace id、错误类型与堆栈（`stack_trace` 由 LogstashEncoder 折成单字段）。手机号、邮箱不是落地版强制脱敏项；若某条日志主动打印了全文，后续审查可以收紧，本版不把它写成观测线部署的阻塞项。
...
计划一要求：容量与「是否 100% 保留」等单服务试点拿出 span 数、字节数、每日写入量再下结论。下面是**工作初值**，不是拍板数字。frog 按计划二 §一 第 6 条确认后生效；单服务试点若实测撑不住，只改本节数字，不改字段名。
...
| Jaeger trace | 7 天（工作目标） | 计划二要求 frog 核对现网 Jaeger 的保留策略与 Badger 数据卷。Badger 嵌入存储没有 Cassandra 那种表级 TTL；实现手段以 frog 现网容器参数与磁盘清理为准，本清单不编造标志名。单服务试点记录每日写入量后由 frog 确认能否维持 7 天。 |
| 采集器持久队列 | 缓冲，不是保留策略 | 短故障用。应用本地 3 天文件才是原始副本。自动重放步骤属后置项。 |
| 部署单与删除墓碑 | 随 Beta 部署与流量治理合同 | 观测后端里该 deployment 的日志 / trace **不随 beta 部署删除**（事后排障还要查），到期由本表自动清除。 |
...
- **按 deployment 删容器**：部署控制器与 Beta 部署合同的安全删除只清理容器、卷、部署目录。观测后端数据保留到到期。
...
## 10. 给观测线部署的验收对照（单服务试点 / 落地版 §1.6 会用到的字段项）
...
6. 坏 JSON 行带着 `parse.error` 到达 VictoriaLogs（落地版 §1.4；本清单不改这条，由采集器配置与单服务试点执行）。
...
- `docker-compose.yml` 的具体补丁（compose 全程单写者）。
- 11 份 `logback-spring.xml` 除 `customFields` 以外的滚动参数（日志 JSON 化按落地版 §1.3 抄；滚动数字已在第 8 节引用）。
...
未确认前，观测线部署可以按本清单实现字段与写入侧脱敏；后端保留天数与删除开关以 101 现网为准，单服务试点记录现状。
