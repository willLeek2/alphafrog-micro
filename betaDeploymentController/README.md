...
每个服务还需要配置一个已经存在的环境文件，以及部署时必须挂载的业务卷。环境文件由运维按服务摘取需要的变量后放在控制器主机上，部署单不能提交任意环境变量或挂载路径。文件必须是普通文件、不是符号链接，权限不得宽于 `0600`，每个服务使用不同路径，文件名不能是整份生产 `.env`。控制器生成专用 Compose 之后会核对有效配置里的 `env_file` 恰好指向该服务文件，并把 `.env` 当作数据卷挂进去的配置一并拒绝。
