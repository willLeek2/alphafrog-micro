 ...
 * 夹具里冻结的模型回合清单。
 *
 * <p>一条 Run 的模型调用按这份清单的顺序一次一次消费，所以它是一份有序的回合表。一个回合里写什么
 * 有明确的口径：{@code text} 与 {@code toolCalls} 至少写一个，两个都写也可以（真实模型的回复本来
 * 就可能既有话要说、又同时要调工具，夹具要能造出这种形态）；两个都不写就拒绝，因为读不出模型要
 * 说什么。工具调用的身份、工具名、参数与先后顺序都照夹具写的样子交出去，执行路径该做的解析、校验、
 * 派发一件不少。</p>
 *
 * <p>要写几个回合，取决于这条 Run 会问几次模型。夹具 Run 跑在「一个分段一次模型调用」的调度器版本上
 * （见 {@code AcceptanceFixtureModelRegistry} 的前置核对），这条路上的调用顺序是：</p>
 * <ul>
 *   <li>规划两次：先策略、再待办清单（结构化规划的那次开关默认开着）。整段规划读不过校验时两次一起
 *       重来，默认最多两次尝试，所以最坏情况占四个回合。</li>
 *   <li>每个节点分段一次：这一段里模型说的话就是这一段要干的工具调用；这一段因为长工具挂起、或者
 *       进程退出后重领，都会再取一个回合。空回复不算一次补救，直接按节点失败收场。</li>
 *   <li>写答案一次：拿不到答案就是失败，不会重问。</li>
