 ...
 * 必答回合必须恰好被认领过一次。</p>
 *
 * <p>放行策略那一路同理：规则写错了字段名或序号时一声不响，一条成员都打不中，那次验收看起来也像
 * 跑完了。所以点名的规则里一条都没打中的也要点名出来。两路结论共用一个字段：这次验收要求的调用
 * 路径与点名事件没有全发生。</p>
 *
 * <p>结果是纯计算：给一份脚本、一份规则清单、这条 Run 已经认领的回合号与已经打中过的规则号，
 * 得出结论、没发生的必答声明、没打中的规则、以及被领走的声明清单。读库与落库在外面
 * （{@link FixtureCallStore}），这里只做判断，好照着例子核对。</p>
 *
...
public record FixtureScenarioVerdict(String verdict,
    ...
    /** 必答声明都发生过、点名的规则都打中过：这次验收要求的调用路径跑全了。 */
    public static final String COMPLETE = "complete";
    /** 有必答声明没发生、或者点名的规则一条都没打中：这一次验收不能算通过。 */
    ...
    public static FixtureScenarioVerdict evaluate(FrozenModelScript script, Set<Integer> claimedTurns) {
        return evaluate(script.declarations(), List.of(), claimedTurns, Set.of());
    }
    ...
    public static FixtureScenarioVerdict evaluate(List<FrozenModelScript.TurnDeclaration> declarations,
        ...
        return evaluate(declarations, List.of(), claimedTurns, Set.of());
    }
     ...
     * @param hitRuleIndexes 真的打中过成员的规则号
     */
    public static FixtureScenarioVerdict evaluate(List<FrozenModelScript.TurnDeclaration> declarations,
                                                  List<FixtureRuleHitStore.RuleFact> ruleFacts,
                                                  Set<Integer> claimedTurns,
                                                  Set<Integer> hitRuleIndexes) {
        Set<Integer> claimed = claimedTurns == null ? Set.of() : new LinkedHashSet<>(claimedTurns);
        Set<Integer> hitRules = hitRuleIndexes == null ? Set.of() : new LinkedHashSet<>(hitRuleIndexes);
        ...
        for (FixtureRuleHitStore.RuleFact fact : ruleFacts) {
            if (!hitRules.contains(fact.index())) {
        ...
        return new FixtureScenarioVerdict(everythingHappened ? COMPLETE : SCRIPT_INCOMPLETE,
                List.copyOf(missing), List.copyOf(missingRules), List.copyOf(consumed));
    }
