 ...
 *
 * <p>这份顺序要对得上，需要夹具泳道把「同一条 Run 同时未完成的工作项」设成 1（泳道热配置）：
 * 那样一条 Run 的节点按计划顺序一段一段来，脚本的先后就是模型被调用的先后。同一条 Run 上
 * 真的并行跑两段时，谁先取回合由线程调度决定，脚本就说不清哪一回合是哪个节点要的。同一段
 * 被重做（进程退出后重领）会再取一个回合，夹具作者要把这种情况也写进脚本；没写到就按
 * 脚本用完报错，不静默补一个回复。</p>
 *
